...
This tells us that we are looking at more than 18k Airbnb rentals in London, for which we have 74 variables. "Glimpse" also tells us that the variables are in all kinds of formats and likely require some manipulation for the actual analysis. For instance, "host_acceptance_rate" is in <chr> format even though it is clearly a numeric variable. 

```{r glimpse, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
#we use glimpse to get an overview of the data
#"kable" is used throughout the code to get clean html tables
...
We can answer the following questions on this model:
...
prop_type_simplifiedEntire rental unit: -13.3071
prop_type_simplifiedEntire serviced apartment: 52.17979
prop_type_simplifiedOther:-13.1395
prop_type_simplifiedPrivate room in rental unit: -41.5226
...
popular_room_type <- cost_four_nights %>% 
  group_by(room_type) %>% 
  summarise(count = n()) %>% 
  arrange(desc(count)) %>% 
...
model2_test<- lm(log_price_four_nights~prop_type_simplified+number_of_reviews+review_scores_rating+room_type,data = cost_four_nights)
...
model2<- lm(log_price_four_nights~+number_of_reviews+review_scores_rating+room_type,data = cost_four_nights)
msummary(model2)
...
model3 <- lm(log_price_four_nights~+number_of_reviews+review_scores_rating+room_type+bedrooms+beds+accommodates,data = cost_four_nights)
...
model4 <- lm(log_price_four_nights~number_of_reviews+review_scores_rating+room_type+bedrooms+beds+accommodates+host_is_superhost,data = cost_four_nights)
...
model5<- lm(log_price_four_nights~number_of_reviews+review_scores_rating+room_type+bedrooms+beds+accommodates+host_is_superhost+instant_bookable,data = cost_four_nights)
...
cost_four_nights_test <- cost_four_nights %>% 
...
result <- c()
for (i in cost_four_nights$neighbourhood_group_cleansed){
  word <- ''
...
cost_four_nights$areas<- result

#running regression model6
model6<- lm(log_price_four_nights~number_of_reviews+review_scores_rating+room_type+bedrooms+beds+accommodates+host_is_superhost+instant_bookable+areas,data = cost_four_nights)
...
model7_test <- lm(log_price_four_nights~number_of_reviews+review_scores_rating+room_type+bedrooms+beds+accommodates+host_is_superhost+instant_bookable+reviews_per_month+availability_30+areas,data = cost_four_nights)
msummary(model7_test)

# As variable reviews_per_month does not seem to be a significant predictor, we remove it from model 7
model7 <- lm(log_price_four_nights~number_of_reviews+review_scores_rating+room_type+bedrooms+accommodates+host_is_superhost+instant_bookable+availability_30+areas,data = cost_four_nights)
...
1. Finally, you must use the best model you came up with for prediction. Suppose you are planning to visit the city you have been assigned to over reading week, and you want to stay in an Airbnb. Find Airbnb's in your destination city that are apartments with a private room, have at least 10 reviews, and an average rating of at least 90. Use your best model to predict the total cost to stay at this Airbnb for 4 nights. Include the appropriate 95% interval with your prediction. Report the point prediction and interval in terms of `price_4_nights`. 
  ...
  We apply the following criteria to find our target listings: 
...
We believe the best model is model 7 as it has the highest adjusted square and the lowest residual SE. We will apply model 7 for price prediction.
