...
model7 <- lm(log_price_four_nights~number_of_reviews+review_scores_rating+room_type+bedrooms+accommodates+host_is_superhost+instant_bookable+availability_30+areas,data = train)
...
Next to the looking at explanatory power, we should also analyze our model using RMSE. This analysis reveals whether the model actually works on unknown data, or whether it is overfitted to the specifics of the training data. The analysis below proves that model 7 is a good model based on two things. First, rmse_train value is small (0.4192), which means predicated value and actual value are pretty close. Second, the difference (0.0016) between rmse_train and rmse_test is small, which means it is a generalized model and can be applied to not only the training set, but also has the predict power to new data. 
